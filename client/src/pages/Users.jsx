...
class Users extends Component {
    ...
    componentDidMount() {
        axios.get('http://localhost:4521/api/users/').then(({ data }) => this.setState({ users: data }));
    ...
    deleteUser = id => {
        ...
        axios.delete('http://localhost:4521/api/users/' + id, {
            headers: {
                'x-access-token': cookie.load('x-access-token')
            }
        }).then(({ data }) => axios.get('http://localhost:4521/api/users/').then(({ data }) => {
